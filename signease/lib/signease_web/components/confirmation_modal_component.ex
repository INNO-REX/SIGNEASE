defmodule SigneaseWeb.Components.ConfirmationModalComponent do
  use Phoenix.LiveComponent

  def render(assigns) do
    ~H"""
    <div
      id={@id}
      class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4"
      phx-click="close_confirmation_modal"
      phx-target={@myself}
    >
      <div
        class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4"
        phx-click-away="close_confirmation_modal"
        phx-target={@myself}
      >
        <div class="p-6">
          <!-- Icon -->
          <div class="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-red-100 mb-4">
            <svg class="h-6 w-6 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
                stroke-width="2"
                d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z"
              />
            </svg>
          </div>

          <!-- Title -->
          <h3 class="text-lg font-medium text-gray-900 text-center mb-2">
            <%= @title %>
          </h3>

          <!-- Message -->
          <p class="text-sm text-gray-500 text-center mb-6">
            <%= @message %>
          </p>

          <!-- Buttons -->
          <div class="flex justify-center space-x-3">
            <button
              type="button"
              class="inline-flex justify-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
              phx-click="close_confirmation_modal"
              phx-target={@myself}
            >
              Cancel
            </button>
            <button
              type="button"
              class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-red-600 border border-transparent rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
              phx-click="confirm_action"
              phx-target={@myself}
            >
              <%= @confirm_text %>
            </button>
          </div>
        </div>
      </div>
    </div>
    """
  end

  def update(assigns, socket) do
    {:ok,
     socket
     |> assign(:id, assigns.id)
     |> assign(:title, assigns.title)
     |> assign(:message, assigns.message)
     |> assign(:confirm_text, assigns.confirm_text)
     |> assign(:action, assigns.action)
     |> assign(:action_params, assigns.action_params)}
  end

  def handle_event("close_confirmation_modal", _params, socket) do
    send(self(), {:close_confirmation_modal, socket.assigns.id})
    {:noreply, socket}
  end

  def handle_event("confirm_action", _params, socket) do
    send(self(), {:confirm_action, socket.assigns.action, socket.assigns.action_params})
    # Close the modal immediately after confirming
    send(self(), {:close_confirmation_modal, socket.assigns.id})
    {:noreply, socket}
  end
end
